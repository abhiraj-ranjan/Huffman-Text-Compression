...
for _ in range(n - 1):
    ...
    if not w: g[u].append(v); g[v].append(u)
...
arr = input()
tail, head = 0, -1
hash_map = [0 for _ in range(123)]
ans = 0
while tail < n:
    while head + 1 < n and hash_map[ord(arr[head + 1])] == 0:
        head += 1
        hash_map[ord(arr[head])] = 1
    tmp = head - tail + 1
    ans += tmp * (tmp + 1) // 2
    if tail > head:
        tail += 1
